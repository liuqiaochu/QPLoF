...
// calculate the coefficient process parameter via MM algorithm
SEXP compbmmc(SEXP X, SEXP Y, SEXP TAU, SEXP THETA, SEXP N, SEXP PM, SEXP KN, SEXP MAXITER, SEXP TOL, SEXP EPSILON)
{
    // X: predictor matrix
    // Y: response vector
    // TAU: quantile level tau vector
    // THETA: starting point of the coef
    // N: sample size
    // PM: p*m, where m is the number of basis functions
    // KN: number of TAUs
    // MAXITER: max number of iterations
    // TOL: tolerance of convergence
    // EPSILON: parameter used in MM
    ...
    double epsilon=REAL(EPSILON)[0];
    double **x, *y, *tau, *theta;
    double err=100.0;   // sum of absolute difference between theta and theta.old
    double *r;   // residual vector
    // delta is the solution for  X^TWX*delta = X^T*V
    // by design of the lubksb, the value of delta will be stored in xv
    // theta.new = theta.old - delta = theta.old - xv
...
SEXP unbc(SEXP X, SEXP X_B, SEXP PHI, SEXP PHI_B, SEXP N, SEXP P)
{
    // X: original observation
    // X.B: bootstrap observation
    // PHI: function Phi
    // PHI.B: function Phi with respect to X.B
    // N: sample size
    ...
    double dis=0;   // store the distance between 2 predictors
